...
public final class CipherMetricsStatsLog {
    ...
    private Map<MetricsCipher, Integer> cipherMap = new HashMap<>();
    ...
    private void writeCipherMap() {
        for (MetricsCipher cipher : cipherMap.keySet()) {
            ConscryptStatsLog.write(ConscryptStatsLog.CONSCRYPT_CIPHER_USED, cipher.getId(), cipherMap.get(cipher));
        }
    }
}
