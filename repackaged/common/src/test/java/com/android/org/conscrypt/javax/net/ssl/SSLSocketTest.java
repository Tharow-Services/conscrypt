...
import com.android.org.conscrypt.SpakeKeyManager;
...
public class SSLSocketTest {
    ...
    @Test
    public void testSpake() throws Exception {
        ...
        SpakeKeyManagerParameters kmfParamsClient = new SpakeKeyManagerParameters.Prover
            .Builder(password)
            .setContext(context)
            .build();
        ...
        assertTrue(keyManagersClient[0] instanceof SpakeKeyManager);
        SpakeKeyManager spakeKeyManagerClient = (SpakeKeyManager) keyManagersClient[0];
        assertTrue(spakeKeyManagerClient.isClient());

        SSLContext contextClient = SSLContext.getInstance("TlsV1.3");
        contextClient.init(keyManagersClient, tmf.getTrustManagers(), null);

        SpakeKeyManagerParameters kmfParamsServer = new SpakeKeyManagerParameters.Verifier
            .Builder(password)
            .setContext(context)
            .build();
        ...
        assertTrue(keyManagersServer[0] instanceof SpakeKeyManager);
        SpakeKeyManager spakeKeyManagerServer = (SpakeKeyManager) keyManagersServer[0];
        ...
        SSLServerSocket serverSocket =
                (SSLServerSocket) contextServer.getServerSocketFactory()
                        .createServerSocket();
        serverSocket.bind(new InetSocketAddress(hostS, 0));
        SSLSocket server = (SSLSocket) serverSocket.accept();
        SSLSocket client = (SSLSocket) contextClient.getSocketFactory().createSocket(hostC, server.getLocalPort());
        ...
        socketWrite(client, "hello".getBytes());
        socketWrite(server, "world".getBytes());
        assertEquals(5, client.getInputStream().read(buffer));
        assertEquals(5, server.getInputStream().read(buffer));
        try {
            client.close();
            server.close();
        } catch (IOException e) {
    ...
    @Test
    public void testSpakeAndOthersInvalid() throws Exception {
        byte[] password = "password".getBytes();
        byte[] context = "context".getBytes();

        SpakeKeyManagerParameters spakeParams =
                new SpakeKeyManagerParameters.Verifier
                        .Builder(password)
                        .setContext(context)
                        .build();
    ...
    @Test
    public void testSpakeNoTrustOrKeyInvalid() throws Exception {
        byte[] password = "password".getBytes();
        byte[] context = "context".getBytes();
