...
public class NativeCryptoTest extends TestCase {
    ...
    public void test_SSL_do_handshake_optional_client_certificate() throws Exception {
        ...
        boolean sawRSA = false, sawEC = false;
        for (int i = 0; i < clientCallback.keyTypes.length; i++) {
            String type = SSLParametersImpl.getClientKeyType(clientCallback.keyTypes[i]);
            if (type == "RSA") {
                sawRSA = true;
            } else if (type == "EC") {
                sawEC = true;
