...
static jlong NativeCrypto_SSL_do_handshake_bio(JNIEnv* env, jclass, jlong ssl_address,
    ...
    JNI_TRACE(
            "ssl=%p NativeCrypto_SSL_do_handshake_bio rbio=%p wbio=%p shc=%p",
            ssl, rbio, wbio, shc);
    if (ssl == nullptr) {
        return 0;
    }
    ...
    if (alpn_protocols != nullptr) {
        if (client_mode) {
            ...
            if (protosBytes.get() == nullptr) {
                JNI_TRACE("ssl=%p NativeCrypto_SSL_configure_alpn alpn_protocols=%p => "
                          "protosBytes == null",
                          ssl,
                          alpn_protocols);
                return;
...
static jint NativeCrypto_ENGINE_SSL_read_heap(JNIEnv* env, jclass, jlong sslRef, jbyteArray destJava,
                                              jint destOffset, jint destLength, jobject shc) {
    SSL* ssl = to_SSL(env, sslRef, true);
    if (ssl == nullptr) {
        return -1;
    }
    ...
    if (ARRAY_OFFSET_LENGTH_INVALID(dest, destOffset, destLength)) {
        JNI_TRACE("ssl=%p NativeCrypto_ENGINE_SSL_read_heap => destOffset=%d, destLength=%d, "
                  "size=%zd",
                  ssl, destOffset, destLength, dest.size());
        jniThrowException(env, "java/lang/ArrayIndexOutOfBoundsException", nullptr);
        return -1;
...
static int NativeCrypto_ENGINE_SSL_write_BIO_direct(JNIEnv* env, jclass, jlong sslRef, jlong bioRef,
    ...
    if (shc == nullptr) {
        jniThrowNullPointerException(env, "sslHandshakeCallbacks == null");
        JNI_TRACE("ssl=%p NativeCrypto_ENGINE_SSL_write_BIO_direct => "
                  "sslHandshakeCallbacks == null", ssl);
        return -1;
    }
...
static int NativeCrypto_ENGINE_SSL_write_BIO_heap(JNIEnv* env, jclass, jlong sslRef, jlong bioRef,
    ...
    if (ARRAY_OFFSET_LENGTH_INVALID(source, sourceOffset, sourceLength)) {
        JNI_TRACE("ssl=%p NativeCrypto_ENGINE_SSL_write_BIO_heap => sourceOffset=%d, "
    ...
    int result = BIO_write(bio,
        reinterpret_cast<const char*>(source.get()) + sourceOffset, sourceLength);
    appData->clearCallbackState();
    return result;
}
...
static int NativeCrypto_ENGINE_SSL_read_BIO_heap(JNIEnv* env, jclass, jlong sslRef, jlong bioRef,
    ...
    if (ARRAY_OFFSET_LENGTH_INVALID(dest, destOffset, destLength)) {
        JNI_TRACE("ssl=%p NativeCrypto_ENGINE_SSL_read_BIO_heap => destOffset=%d, destLength=%d, "
                  "size=%zd",
                  ssl, destOffset, destLength, dest.size());
        jniThrowException(env, "java/lang/ArrayIndexOutOfBoundsException", nullptr);
        return -1;
...
static int NativeCrypto_ENGINE_SSL_write_heap(JNIEnv* env, jclass, jlong sslRef,
    ...
    if (ARRAY_OFFSET_LENGTH_INVALID(source, sourceOffset, sourceLength)) {
        JNI_TRACE("ssl=%p NativeCrypto_ENGINE_SSL_write_heap => sourceOffset=%d, sourceLength=%d, "
                  "size=%zd",
                  ssl, sourceOffset, sourceLength, source.size());
        jniThrowException(env, "java/lang/ArrayIndexOutOfBoundsException", nullptr);
        return -1;
    ...
    int result = SSL_write(ssl,
                           reinterpret_cast<const char*>(source.get()) + sourceOffset,
                           sourceLength);
    appData->clearCallbackState();
    return result;
}
