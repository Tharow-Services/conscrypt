 ...
 * in the error stack.
 */
static void safeSslClear(JNIEnv* env, SSL* ssl) {
    SSL_clear(ssl);
    unsigned long error = ERR_peek_last_error();
    if (error == 0 || (ERR_GET_LIB(error) == ERR_LIB_SSL && ERR_GET_FUNC(error) == SSL_F_SSL_CLEAR)) {
        freeOpenSslErrorState();
    } else {
        const char* msg;
        if (env->ExceptionCheck()) {
            env->ExceptionClear();
            msg = "Error during clearing SSL state with pending exception";
        } else {
            msg = "Error during clearing SSL state";
        }
        throwSSLExceptionWithSslErrors(env, ssl, SSL_ERROR_NONE, msg, jniThrowRuntimeException);
    }
}
