...
static jlong NativeCrypto_HMAC_CTX_new(JNIEnv* env, jclass) {
    ...
    HMAC_CTX* hmacCtx = reinterpret_cast<HMAC_CTX*>(
...
static void NativeCrypto_HMAC_Init_ex(JNIEnv* env, jclass, jobject hmacCtxRef,
        jbyteArray keyArray, jobject evpMdRef) {
    ...
    if (!HMAC_Init_ex(hmacCtx, keyPtr, keyBytes.size(), md, NULL)) {
        ...
        JNI_TRACE("HMAC_Init_ex(%p, %p, %p) => fail HMAC_Init_ex", hmacCtx,
                  keyArray, md);
        return;
    }
}
