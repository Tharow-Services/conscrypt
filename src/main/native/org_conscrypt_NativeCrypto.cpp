        ...
        jlong ssl_address)
{
    ...
    if (byteArray.get() == NULL) {
        JNI_TRACE("NativeCrypto_SSL_get_ocsp_response(%p) => "
                  "creating byte array failed", ssl);
        return NULL;
    }

    env->SetByteArrayRegion(byteArray.get(), 0, data_len, (const jbyte*)data);
    JNI_TRACE("NativeCrypto_SSL_get_ocsp_response(%p) => "
              "%p [size=%zd]", ssl, byteArray.get(), data_len);
...
{
    SSL_CTX* ssl_ctx = to_SSL_CTX(env, ssl_ctx_address, true);
    JNI_TRACE("ssl_ctx=%p NativeCrypto_SSL_CTX_set_ocsp_response",
            ssl_ctx);
    if (ssl_ctx == NULL)  {
        return;
    }
    ...
    if (!SSL_CTX_set_ocsp_response(ssl_ctx,
        ...
        jniThrowRuntimeException(env, "SSL_CTX_set_ocsp_response failed");
    } else {
 ...
 */
static bool match_ocsp_cert_id(CBS *cert_id, X509 *x509, X509 *issuerX509) {
 ...
 * error occured or no response matches the certificate, false is returned.
 */
static bool find_ocsp_single_response(CBS* responses, X509 *x509, X509 *issuerX509,
    ...
    while (CBS_len(responses) > 0) {
        ...
        if (match_ocsp_cert_id(&cert_id, x509, issuerX509)) {
 ...
 * If parsing succeeds, |responses| is set to point to the sequence of SingleResponse, and true is
 * returned.
 */
