...
static jstring NativeCrypto_get_cipher_names(JNIEnv *env, jclass, jstring selectorJava) {
    ...
    size_t len = 0;
    for (size_t i = 0; i < (size_t) sk_SSL_CIPHER_num(ciphers); i++) {
    ...
    UniquePtr<char[]> ret(new char[len]);
    size_t j = 0;
    ...
    for (size_t i = 0; i < sk_SSL_CIPHER_num(ciphers); i++) {
        ...
        memcpy(&ret[j], name, name_len);
        j += name_len;
    }

    ret[j] = 0;
    return env->NewStringUTF(ret.get());
}
