 ...
 */
#define ARRAY_CHECK_OFFSET(array, offset) (offset < 0 || offset > ssize_t(array.size()))

/**
 * Check array bounds for arguments when an array, offset, and length are given.
 */
#define ARRAY_CHECK_OFFSET_LENGTH(array, offset, len) (offset < 0 || \
        offset > ssize_t(array.size()) || len < 0 || len > ssize_t(array.size()) - offset)
...
static jint evp_aead_ctx_op(JNIEnv* env, jobject ctxRef, jbyteArray outArray, jint outOffset,
    ...
    if (ARRAY_CHECK_OFFSET(outBytes, outOffset)) {
    ...
    if (ARRAY_CHECK_OFFSET_LENGTH(inBytes, inOffset, inLength)) {
        JNI_TRACE("evp_aead_ctx_op(%p, %p, %d, %p, %d, %d, %p)", ctx, outArray, outOffset,
                nonceArray, inArray, inOffset, inLength, aadArray);
...
static jint NativeCrypto_EVP_AEAD_CTX_seal(JNIEnv* env, jclass, jobject ctxRef, jbyteArray outArray,
        ...
        jbyteArray aadArray) {
#if defined(OPENSSL_IS_BORINGSSL)
    return evp_aead_ctx_op(env, ctxRef, outArray, outOffset, nonceArray, inArray, inOffset, inLength,
