...
static void debug_print_session_key(SSL* ssl) {
    ...
    if (EVP_PKEY_type(pkey->type) == EVP_PKEY_RSA) {
        ...
        client_random = new uint8_t[max_out];
        SSL_get_client_random(ssl, client_random, max_out);
        hex_string(client_random_str, client_random, max_out);
