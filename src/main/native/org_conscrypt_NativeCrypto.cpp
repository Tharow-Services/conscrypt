...
static jlong NativeCrypto_EVP_AEAD_CTX_init(JNIEnv* env, jclass, jlong evpAeadRef,
    ...
    Unique_EVP_AEAD_CTX aeadCtx(reinterpret_cast<EVP_AEAD_CTX*>(OPENSSL_malloc(sizeof(EVP_AEAD_CTX))));
    memset(aeadCtx.get(), 0, sizeof(EVP_AEAD_CTX));
    const uint8_t* tmp = reinterpret_cast<const uint8_t*>(keyBytes.get());
    int ret = EVP_AEAD_CTX_init(aeadCtx.get(), evpAead, tmp, keyBytes.size(), tagLen,
            NULL);
    if (ret != 1) {
    ...
    JNI_TRACE("EVP_AEAD_CTX_init(%p, %p, %d) => %p", evpAead, keyArray, tagLen,
            aeadCtx.get());
...
static jlong NativeCrypto_EVP_AEAD_CTX_seal(JNIEnv* env, jclass, jobject ctxRef, jbyteArray outArray,
    ...
    JNI_TRACE("EVP_AEAD_CTX_seal(%p, %p, %d, %p, %p, %d, %d, %p)", ctx, outArray, outOffset,
    ...
    int ret = EVP_AEAD_CTX_seal(ctx, outTmp + outOffset, &actualOutLength,
            outBytes.size() - outOffset, nonceTmp, nonceBytes.size(), inTmp + inOffset,
            inLength, aad_chars, aad_chars_size);
    if (ret != 1) {
        throwExceptionIfNecessary(env, "EVP_AEAD_CTX_seal");
    }

    JNI_TRACE("EVP_AEAD_CTX_seal(%p, %p, %d, %p, %p, %d, %d, %p) => ret=%d, outLength=%zd",
...
static jint NativeCrypto_EVP_AEAD_CTX_open(JNIEnv* env, jclass, jobject ctxRef, jbyteArray outArray,
...
#if defined(OPENSSL_IS_BORINGSSL)
    EVP_AEAD_CTX* ctx = fromContextObject<EVP_AEAD_CTX>(env, ctxRef);
    JNI_TRACE("EVP_AEAD_CTX_open(%p, %p, %d, %p, %p, %d, %d, %p)", ctx, outArray, outOffset,
            nonceArray, inArray, inOffset, inLength, aadArray);
    ...
    const uint8_t* nonceTmp = reinterpret_cast<const uint8_t*>(nonceBytes.get());
    size_t actualOutLength;
    int ret = EVP_AEAD_CTX_open(ctx, outTmp + outOffset, &actualOutLength,
            outBytes.size() - outOffset, nonceTmp, nonceBytes.size(), inTmp + inOffset,
            inLength, aad_chars, aad_chars_size);
    if (ret != 1) {
        throwExceptionIfNecessary(env, "EVP_AEAD_CTX_open");
    }

    JNI_TRACE("EVP_AEAD_CTX_open(%p, %p, %d, %p, %p, %d, %d, %p) => ret=%d, outLength=%zd",
            ctx, outArray, outOffset, nonceArray, inArray, inOffset, inLength, aadArray, ret,
            actualOutLength);
    return static_cast<jlong>(actualOutLength);
#else
