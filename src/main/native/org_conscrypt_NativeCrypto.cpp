...
static jclass nativeCryptoClass;
...
static jbyteArray rawSignDigestWithPrivateKey(JNIEnv* env, jobject privateKey,
    ...
    jmethodID rawSignMethod = env->GetStaticMethodID(nativeCryptoClass,
    ...
    return reinterpret_cast<jbyteArray>(env->CallStaticObjectMethod(
            nativeCryptoClass, rawSignMethod, privateKey, messageArray.get()));
}

static jbyteArray rawDecryptWithPrivateKey(JNIEnv* env, jobject privateKey,
        const char* ciphertext, size_t ciphertext_len) {
    ScopedLocalRef<jbyteArray> ciphertextArray(env, env->NewByteArray(ciphertext_len));
    if (env->ExceptionCheck()) {
        JNI_TRACE("rawDecryptWithPrivateKey(%p) => threw exception", privateKey);
        return NULL;
    }

    ScopedByteArrayRW ciphertextBytes(env, ciphertextArray.get());
    if (ciphertextBytes.get() == NULL) {
        JNI_TRACE("rawDecryptWithPrivateKey(%p) => using byte array failed", privateKey);
        return NULL;
    }
    ...
    jmethodID rawSignMethod = env->GetStaticMethodID(nativeCryptoClass,
            "rawDecryptWithPrivateKey", "(Ljava/security/PrivateKey;[B)[B");
    if (rawSignMethod == NULL) {
        ALOGE("Could not find rawDecryptWithPrivateKey");
        return NULL;
    }

    return reinterpret_cast<jbyteArray>(env->CallStaticObjectMethod(
            nativeCryptoClass, rawSignMethod, privateKey, ciphertextArray.get()));
}
...
int RsaMethodPrivEnc(int flen,
    ...
    ScopedLocalRef<jbyteArray> signature(
            env, rawSignDigestWithPrivateKey(env, private_key, reinterpret_cast<const char*>(from),
                                             flen));
...
int RsaMethodPrivDec(int flen,
    ...
    ScopedLocalRef<jbyteArray> cleartext(
            env,
            rawDecryptWithPrivateKey(env, private_key, reinterpret_cast<const char*>(from), flen));
...
static void initialize_conscrypt(JNIEnv* env) {
    ...
    nativeCryptoClass = getGlobalRefToClass(env,
