...
public class OpenSSLEngineImpl extends SSLEngine implements NativeCrypto.SSLHandshakeCallbacks {
    ...
    public SSLEngineResult unwrap(ByteBuffer src, ByteBuffer[] dsts, int offset, int length)
        ...
        System.out.println("unwrap, actual status: " + handshakeStatus);
        if (handshakeStatus == HandshakeStatus.NEED_TASK) {
            return new SSLEngineResult(Status.OK, handshakeStatus, 0, 0);
        } else if (handshakeStatus == HandshakeStatus.NEED_UNWRAP) {
            System.out.println("trying to unwrap");
        ...
        // TODO implement SSL_read
        throw new UnsupportedOperationException("haven't implemented reading yet");
    }
    ...
    public SSLEngineResult wrap(ByteBuffer[] srcs, int offset, int length, ByteBuffer dst)
        ...
        // If we haven't completed the handshake yet, just let the caller know.
        HandshakeStatus handshakeStatus = getHandshakeStatus();
        System.out.println("wrap, actual status: " + handshakeStatus);
        if (handshakeStatus == HandshakeStatus.NEED_TASK) {
            return new SSLEngineResult(Status.OK, handshakeStatus, 0, 0);
            ...
            int toWrite = Math.min(handshakeSink.available(), dst.remaining());
            dst.put(handshakeSink.toByteArray(), handshakeSink.position(), toWrite);
            handshakeSink.skip(toWrite);
            return new SSLEngineResult(Status.OK, getHandshakeStatus(), 0, 0);
        }

        // TODO implement SSL_write
        throw new UnsupportedOperationException("haven't implemented writing yet");
    }
