 ...
 */
public final class OpenSSLEngineImpl extends SSLEngine implements
    NativeCrypto.SSLHandshakeCallbacks,
    SSLParametersImpl.AliasChooser, SSLParametersImpl.PSKCallbacks {

    private static final SSLEngineResult NEED_UNWRAP_OK = new SSLEngineResult(OK, NEED_UNWRAP, 0,
        0);
    private static final SSLEngineResult NEED_UNWRAP_CLOSED = new SSLEngineResult(CLOSED,
        NEED_UNWRAP,
        0, 0);
    private static final SSLEngineResult NEED_WRAP_OK = new SSLEngineResult(OK, NEED_WRAP, 0, 0);
    private static final SSLEngineResult NEED_WRAP_CLOSED = new SSLEngineResult(CLOSED, NEED_WRAP,
        0,
        0);
    private static final SSLEngineResult CLOSED_NOT_HANDSHAKING = new SSLEngineResult(CLOSED,
        NOT_HANDSHAKING, 0, 0);
    ...
    public SSLEngineResult unwrap(
        ...
        synchronized (stateLock) {
            ...
            if (len < packetLength) {
                // We either have not enough data to read the packet header or not enough for reading
                // the whole packet.
                return new SSLEngineResult(BUFFER_UNDERFLOW, getHandshakeStatus(), 0, 0);
            }
            ...
            if (pendingInboundCleartextBytes(handshakeStatus) > 0) {
                ...
                return new SSLEngineResult(
                    BUFFER_OVERFLOW, mayFinishHandshake(
                    handshakeStatus == FINISHED ? handshakeStatus : getHandshakeStatusInternal()),
    ...
    private SSLEngineResult.HandshakeStatus handshake() throws SSLException {
        ...
        try {
            ...
            sslSession = sslParameters
                .setupSession(sslSessionCtx, sslNativePointer, sslSession,
    ...
    private int writePlaintextData(final ByteBuffer src) throws SSLException {
        try {
            ...
            if (src.isDirect()) {
                ...
                sslWrote = NativeCrypto
                    .ENGINE_SSL_write_heap(sslNativePointer, src.array(), src.arrayOffset() + pos,
                        len,
                        this);
            }
            if (sslWrote > 0) {
    ...
    private int readPlaintextData(final ByteBuffer dst) throws SSLException {
        try {
            ...
            if (dst.isDirect()) {
                ...
                sslRead = NativeCrypto
                    .ENGINE_SSL_read_heap(sslNativePointer, dst.array(), dst.arrayOffset() + pos,
                        len,
                        this);
                if (sslRead > 0) {
                    dst.position(pos + sslRead);
    ...
    private int writeEncryptedData(final ByteBuffer src, int len) throws SSLException {
        try {
            ...
            if (src.isDirect()) {
                long addr = NativeCrypto.getDirectBufferAddress(src) + pos;
                netWrote = NativeCrypto
                    .ENGINE_SSL_write_BIO_direct(sslNativePointer, networkBio, addr, len, this);
            } else {
                netWrote = NativeCrypto
                    .ENGINE_SSL_write_BIO_heap(sslNativePointer, networkBio, src.array(),
                        src.arrayOffset() + pos, len, this);
            }
    ...
    private SSLEngineResult readPendingBytesFromBIO(ByteBuffer dst, int bytesConsumed,
        ...
        if (pendingNet > 0) {
            ...
            return new SSLEngineResult(getEngineStatus(),
                mayFinishHandshake(status == FINISHED ? status : getHandshakeStatus(pendingNet)),
                bytesConsumed, bytesProduced);
        }
        return null;
    }
    ...
    private int readEncryptedData(final ByteBuffer dst, final int pending) throws SSLException {
        try {
            ...
            if (dst.remaining() >= pending) {
                ...
                if (dst.isDirect()) {
                    long addr = NativeCrypto.getDirectBufferAddress(dst) + pos;
                    bioRead = NativeCrypto
                        .ENGINE_SSL_read_BIO_direct(sslNativePointer, networkBio, addr, len, this);
                    if (bioRead > 0) {
                    ...
                    bioRead = NativeCrypto
                        .ENGINE_SSL_read_BIO_heap(sslNativePointer, networkBio, dst.array(),
                            dst.arrayOffset() + pos, pending, this);
                    if (bioRead > 0) {
                        dst.position(pos + bioRead);
                        return bioRead;
                    }
    ...
    private SSLEngineResult.HandshakeStatus mayFinishHandshake(
        ...
        throws SSLException {
        if (!handshakeFinished
            && status == NOT_HANDSHAKING /*|| engineState == EngineState.HANDSHAKE_COMPLETED)*/) {
    ...
    private SSLEngineResult newResult(
        int bytesConsumed, int bytesProduced, SSLEngineResult.HandshakeStatus status)
        throws SSLException {
        return new SSLEngineResult(
            getEngineStatus(),
        ...
        if (offset >= length || offset + count > length) {
            throw new IndexOutOfBoundsException(
                "offset: " + offset + ", length: " + count +
                    " (expected: offset <= offset + length <= srcs.length (" + length + "))");
        }
    }
