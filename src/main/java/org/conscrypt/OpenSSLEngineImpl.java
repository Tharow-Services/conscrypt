...
public final class OpenSSLEngineImpl extends SSLEngine
    ...
    public boolean isInboundDone() {
        ...
        return (NativeCrypto.SSL_get_shutdown(sslNativePointer)
                       & NativeConstants.SSL_RECEIVED_SHUTDOWN)
                != 0;
    }
    ...
    public boolean isOutboundDone() {
        ...
        return (NativeCrypto.SSL_get_shutdown(sslNativePointer) & NativeConstants.SSL_SENT_SHUTDOWN)
                != 0;
    }
    ...
    public SSLEngineResult unwrap(final ByteBuffer[] srcs, int srcsOffset, final int srcsLength,
        ...
        synchronized (stateLock) {
            ...
            if (len < NativeConstants.SSL3_RT_HEADER_LENGTH) {
            ...
            if (packetLength - NativeConstants.SSL3_RT_HEADER_LENGTH > capacity) {
            ...
            if (srcsOffset < srcsEndOffset) {
                ...
                do {
                    ...
                    int written = writeEncryptedData(
                            src, Math.min(packetLengthRemaining, src.remaining()));
                    if (written > 0) {
    ...
    private int writePlaintextData(final ByteBuffer src) throws SSLException {
        try {
            ...
            if (src.isDirect()) {
                ...
                sslWrote = NativeCrypto.ENGINE_SSL_write_heap(
                        sslNativePointer, src.array(), src.arrayOffset() + pos, len, this);
            }
            if (sslWrote > 0) {
        ...
        try {
            ...
            if (src.isDirect()) {
                ...
                netWrote = NativeCrypto.ENGINE_SSL_write_BIO_heap(sslNativePointer, networkBio,
                        src.array(), src.arrayOffset() + pos, len, this);
            }
    ...
    private static void checkIndex(int length, int offset, int count, String arrayName) {
        if (offset >= length || offset + count > length) {
            throw new IndexOutOfBoundsException("offset: " + offset + ", length: " + count
