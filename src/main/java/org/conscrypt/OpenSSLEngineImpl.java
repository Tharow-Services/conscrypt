...
public class OpenSSLEngineImpl extends SSLEngine implements NativeCrypto.SSLHandshakeCallbacks,
    ...
    public SSLEngineResult unwrap(ByteBuffer src, ByteBuffer[] dsts, int offset, int length)
        ...
        if (handshakeStatus == HandshakeStatus.NEED_TASK) {
            ...
            try {
                long sslSessionCtx = NativeCrypto.SSL_do_handshake_bio(sslNativePointer,
                        source.getContext(), handshakeSink.getContext(), this, getUseClientMode(),
                        sslParameters.npnProtocols, sslParameters.alpnProtocols);
                if (sslSessionCtx != 0) {
                    try {
                        sslParameters.setupSession(sslSessionCtx, sslNativePointer, null,
    ...
    public SSLEngineResult wrap(ByteBuffer[] srcs, int offset, int length, ByteBuffer dst)
        ...
        if (handshakeStatus == HandshakeStatus.NEED_TASK) {
            ...
            if (handshakeSink.available() == 0) {
                ...
                if (sslSessionCtx != 0) {
                    try {
                        sslParameters.setupSession(sslSessionCtx, sslNativePointer, null, null,
                                getPeerPort(), true);
                    } catch (IOException e) {
                        throw new SSLHandshakeException(e);
                    }
