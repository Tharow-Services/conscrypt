...
public class CTVerifier {
    ...
    /**
     * Verify a list of signed certificate timestamps for the given certificate chain.
     * Each SCT is verified against the list of known logs.
     * @return CTResults containing both valid and invalid SCTs.
     */
    private CTResults verifySCTs(OpenSSLX509Certificate[] chain, List<SignedCertificateTimestamp> scts)
        ...
        if (chain.length >= 2) {
            ...
            try {
                ...
                // Ignore the error and keep precertEntry null
                // verifySingleSCT simply won't be able to verify embedded scts
            }
        }
    ...
    /**
     * Verify a single SCT.
     * If the SCT was extracted from an X509 extension, precertEntry is used to verify it.
     * Otherwise x509Entry is used.
     *
     * precertEntry may be null, in which case an of the SCT comes from an X509 extension it
     * will be considered invalid.
     *
     * @return SCTVerificationResult instance containing the SCT, the result of the verification,
     *         and the log used to verify it.
     */
     ...
     * If the encompassing encoded list fails to be parsed, null is returned. Individual SCTs which
     * fail to be parsed are ignored.
     ...
     * If the OCSP response does not contain any SCT extension, or the encompassing encoded list
     * fails to be parsed, null is returned. Individual SCTs which fail to be parsed are ignored.
     ...
     *
     * If the certificate does not contain any SCT extension, or the encompassing encoded list
     * fails to be parsed, null is returned. Individual SCTs which fail to be parsed are ignored.
     */
