...
public class CTVerifier {
    ...
    private void verifyEmbeddedSCTs(List<SignedCertificateTimestamp> scts,
        ...
        if (precertEntry == null) {
            markSCTsAsInvalid(scts, results);
            return;
        }

        for (SignedCertificateTimestamp sct: scts) {
            SCTVerificationResult.Status status = verifySingleSCT(sct, precertEntry);
            results.addResult(new SCTVerificationResult(sct, status));
        }
    }
    ...
    private void verifyExternalSCTs(List<SignedCertificateTimestamp> scts,
        ...
        for (SignedCertificateTimestamp sct: scts) {
            SCTVerificationResult.Status status = verifySingleSCT(sct, x509Entry);
            results.addResult(new SCTVerificationResult(sct, status));
        }
    }
     ...
     */
    private SCTVerificationResult.Status verifySingleSCT(SignedCertificateTimestamp sct,
                                                         CertificateEntry certEntry) {
        CTLogInfo log = store.getKnownLog(sct.getLogID());
        if (log == null) {
            return SCTVerificationResult.Status.UNKNOWN_LOG;
        }
    ...
    /**
     * Add every SCT in {@code scts} to {@code results} with INVALID_SCT as status
     */
    private void markSCTsAsInvalid(List<SignedCertificateTimestamp> scts,
                                   CTResults results) {
        for (SignedCertificateTimestamp sct: scts) {
            results.addResult(new SCTVerificationResult(sct,
                        SCTVerificationResult.Status.INVALID_SCT));
        }
    }
