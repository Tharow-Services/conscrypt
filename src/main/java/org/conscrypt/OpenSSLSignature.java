...
public class OpenSSLSignature extends SignatureSpi {
    ...
    private final void resetContext() {
        ...
        NativeCrypto.EVP_MD_CTX_init(ctxLocal.getContext());
        if (signing) {
            enableDSASignatureNonceHardeningIfApplicable();
            NativeCrypto.EVP_SignInit(ctxLocal.getContext(), evpAlgorithm);
        } else {
            NativeCrypto.EVP_VerifyInit(ctxLocal.getContext(), evpAlgorithm);
        }
        this.ctx = ctxLocal;
    }
    ...
    protected void engineUpdate(byte[] input, int offset, int len) {
        ...
        if (signing) {
            NativeCrypto.EVP_SignUpdate(ctxLocal.getContext(), input, offset, len);
        } else {
            NativeCrypto.EVP_VerifyUpdate(ctxLocal.getContext(), input, offset, len);
        }
    }
    ...
    protected byte[] engineSign() throws SignatureException {
        ...
        try {
            ...
            int bytesWritten = NativeCrypto.EVP_SignFinal(ctxLocal.getContext(), buffer, 0,
                    key.getPkeyContext());
    ...
    protected boolean engineVerify(byte[] sigBytes) throws SignatureException {
        ...
        final OpenSSLDigestContext ctxLocal = ctx;
        try {
            int result = NativeCrypto.EVP_VerifyFinal(ctxLocal.getContext(), sigBytes, 0,
