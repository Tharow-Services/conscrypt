...
public class OpenSSLSignature extends SignatureSpi {
    ...
    private final void resetContext() {
        OpenSSLDigestContext ctx = new OpenSSLDigestContext(NativeCrypto.EVP_MD_CTX_create());
        NativeCrypto.EVP_MD_CTX_init(ctx.getContext());
        if (signing) {
            enableDSASignatureNonceHardeningIfApplicable();
            NativeCrypto.EVP_SignInit(ctx.getContext(), evpAlgorithm);
        } else {
            NativeCrypto.EVP_VerifyInit(ctx.getContext(), evpAlgorithm);
        }
        this.ctx = ctx;
    }
    ...
    protected void engineUpdate(byte[] input, int offset, int len) {
        final OpenSSLDigestContext ctx = this.ctx;
        if (signing) {
            NativeCrypto.EVP_SignUpdate(ctx.getContext(), input, offset, len);
        } else {
            NativeCrypto.EVP_VerifyUpdate(ctx.getContext(), input, offset, len);
        }
    }
    ...
    protected byte[] engineSign() throws SignatureException {
        ...
        try {
            ...
            int bytesWritten = NativeCrypto.EVP_SignFinal(ctx.getContext(), buffer, 0,
                    key.getPkeyContext());
    ...
    protected boolean engineVerify(byte[] sigBytes) throws SignatureException {
        ...
        final OpenSSLDigestContext ctx = this.ctx;
        try {
            int result = NativeCrypto.EVP_VerifyFinal(ctx.getContext(), sigBytes, 0,
                    sigBytes.length, key.getPkeyContext());
            return result == 1;
        } catch (Exception ex) {
            return false;
        } finally {
