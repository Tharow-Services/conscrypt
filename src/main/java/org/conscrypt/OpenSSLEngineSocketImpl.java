                ...
                try {
                    ...
                    do {
                        ...
                        if (target.position() != engineResult.bytesProduced()) {
                            throw new SSLException(
                                "Engine bytesProduced " + engineResult.bytesProduced()
    ...
    private final class InputStreamWrapper extends InputStream {
        ...
        private void init() throws IOException {
            if (socketInputStream == null) {
                ...
                socketChannel = socket.getChannel();
                if (socketChannel != null) {
                    fromSocket = ByteBuffer
                        .allocateDirect(engine.getSession().getPacketBufferSize());
                } else {
        ...
        private int readFromSocket() throws IOException {
            ...
            // appropriate.
            int read = socketInputStream
                .read(fromSocket.array(), fromSocket.position(), fromSocket.remaining());
            if (read > 0) {
