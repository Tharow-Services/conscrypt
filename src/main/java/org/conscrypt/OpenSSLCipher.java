...
public abstract class OpenSSLCipher extends CipherSpi {
     ...
     */
    protected abstract void engineInitInternal(int opmode, byte[] encodedKey,
            AlgorithmParameterSpec params, SecureRandom random) throws InvalidKeyException,
    ...
    protected void engineInit(int opmode, Key key, SecureRandom random) throws InvalidKeyException {
        final byte[] encodedKey = checkInitArgs(opmode, key);
        try {
            engineInitInternal(opmode, encodedKey, null, random);
    ...
    protected void engineInit(int opmode, Key key, AlgorithmParameterSpec params,
        ...
        final byte[] encodedKey = checkInitArgs(opmode, key);
        engineInitInternal(opmode, encodedKey, params, random);
    }
    ...
    private byte[] checkInitArgs(int opmode, Key key) throws InvalidKeyException {
    ...
    public static abstract class EVP_CIPHER extends OpenSSLCipher {
         ...
         */
        private EVP_CIPHER_CTX cipherCtx = new EVP_CIPHER_CTX(NativeCrypto.EVP_CIPHER_CTX_new());
        ...
        @Override
        protected void engineInitInternal(int opmode, byte[] encodedKey,
                AlgorithmParameterSpec params, SecureRandom random) throws InvalidKeyException,
                InvalidAlgorithmParameterException {
            byte[] iv;
    ...
    public static abstract class EVP_AEAD extends OpenSSLCipher {
        ...
        protected int updateInternal(byte[] input, int inputOffset, int inputLen, byte[] output,
            ...
            org.conscrypt.util.Arrays.checkOffsetAndCount(input.length, inputOffset, inputLen);
            if (inputLen > 0) {
                expand(inputLen);
        ...
        protected int doFinalInternal(byte[] output, int outputOffset, int maximumLen)
            ...
            final int bytesWritten;
            if (isEncrypting()) {
                bytesWritten = NativeCrypto.EVP_AEAD_CTX_seal(cipherCtx, output, outputOffset, iv,
                        buf, 0, bufCount, aad);
            } else {
                bytesWritten = NativeCrypto.EVP_AEAD_CTX_open(cipherCtx, output, outputOffset, iv,
                        buf, 0, bufCount, aad);
            }
            reset();
            return bytesWritten;
        ...
        protected void engineUpdateAAD(byte[] input, int inputOffset, int inputLen) {
            if (bufCount != 0) {
                throw new IllegalStateException("Cannot call updateAAD after calling update");
            }
            if (aad == null) {
