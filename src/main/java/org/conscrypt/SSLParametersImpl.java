...
public class SSLParametersImpl implements Cloneable {
    ...
    OpenSSLSessionImpl handshakeSession;

    String[] openSslEnabledProtocols;
    String[] openSslEnabledCipherSuites;
    byte[] npnProtocols;
    byte[] alpnProtocols;
    ...
    void setTlsChannelId(long sslNativePointer) throws SSLHandshakeException, SSLException {
        // TLS Channel ID
        if (channelIdEnabled) {
            if (client_mode) {
    ...
    void setSslParameters(long sslCtxNativePointer, long sslNativePointer, Socket socket,
                ...
                for (String keyType : keyTypes) {
                    try {
                        setCertificate(sslNativePointer,
                                keyManager.chooseServerAlias(keyType, null, socket));
    ...
    void setupSession(long sslSessionNativePointer, long sslNativePointer,
    ...
    void setupSSLContext(long sslCtxNativePointer, long sslNativePointer, String hostname, int port)
            throws IOException {
        if (npnProtocols != null) {
            NativeCrypto.SSL_CTX_enable_npn(sslCtxNativePointer);
        }

        final boolean client = getUseClientMode();
    ...
    public void setSession(OpenSSLSessionImpl sslSession) {
        this.sslSession = sslSession;
    }

    public OpenSSLSessionImpl getSession() {
        return sslSession;
    }
