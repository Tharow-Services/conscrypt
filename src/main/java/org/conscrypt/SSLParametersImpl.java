...
public class SSLParametersImpl implements Cloneable {
    ...
    /**
     * Create a CTPolicy instance configured using Security properties.
     * conscrypt.ct.minimumLogCount is used to configure to number of different logs SCTs have to
     * come from for the policy to pass
     */
    private CTPolicy createDefaultCTPolicy() {
        int minimumLogCount = 1;

        String property = Security.getProperty("conscrypt.ct.minimumLogCount");
        if (property != null) {
            try {
                minimumLogCount = Integer.valueOf(property);
            } catch (NumberFormatException e) {
                // Just ignore and use default value
            }
        }

        return new CTPolicyImpl(getDefaultCTLogStore(), minimumLogCount);
    }
}
