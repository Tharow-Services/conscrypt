...
public class OpenSSLMessageDigestJDK extends MessageDigestSpi {
    ...
    private final void resetContext() {
        OpenSSLDigestContext ctxRef = new OpenSSLDigestContext(NativeCrypto.EVP_MD_CTX_create());
        NativeCrypto.EVP_MD_CTX_init(ctxRef.getContext());
        NativeCrypto.EVP_DigestInit(ctxRef.getContext(), evp_md);
        ctx = ctxRef;
    }
    ...
    protected void engineUpdate(byte[] input, int offset, int len) {
        OpenSSLDigestContext ctxRef = ctx;
        NativeCrypto.EVP_DigestUpdate(ctxRef.getContext(), input, offset, len);
    }
    ...
    protected byte[] engineDigest() {
        ...
        NativeCrypto.EVP_DigestFinal(ctxRef.getContext(), result, 0);
        resetContext();
        return result;
    }
