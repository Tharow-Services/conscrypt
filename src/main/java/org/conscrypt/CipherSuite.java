...
public class CipherSuite {
    ...
    private static final Map<String, CipherSuite> SUITES_BY_NAME;
    // hash for quick access to cipher suite by code
    private static final Map<Integer, CipherSuite> SUITES_BY_CODE;
    ...
    static {
        SUITES_BY_NAME = new HashMap<String, CipherSuite>(SUITES.length);
        SUITES_BY_CODE = new HashMap<Integer, CipherSuite>(SUITES.length);
        List<CipherSuite> supportedCipherSuites = new ArrayList<CipherSuite>(SUITES.length);
        List<String> supportedCipherSuiteNames = new ArrayList<String>(SUITES.length);
        CipherSuite nullWithNullNull = null;
        for (CipherSuite cipherSuite : SUITES) {
            byte[] codeBytes = cipherSuite.toBytes();
            if (codeBytes.length != 2) {
                throw new RuntimeException(
                        cipherSuite.getName() + " has unexpected code length: " + codeBytes.length);
            }
            int code = ((codeBytes[0] & 0xff) << 8) | (codeBytes[1] & 0xff);
            if (SUITES_BY_CODE.put(code, cipherSuite) != null) {
                throw new RuntimeException(
                        "Duplicate cipher suite: 0x" + Integer.toHexString(code));
            }

            // Code in this library assumes that SSL_NULL_WITH_NULL_NULL is not listed in
            // SUITES_BY_NAME or the list of supported cipher suites.
            if ("SSL_NULL_WITH_NULL_NULL".equals(cipherSuite.getName())) {
                nullWithNullNull = cipherSuite;
                continue;
            }

            if (SUITES_BY_NAME.put(cipherSuite.getName(), cipherSuite) != null) {
                throw new RuntimeException("Duplicate cipher suite: " + cipherSuite.getName());
            }
            if (cipherSuite.supported) {
                supportedCipherSuites.add(cipherSuite);
                supportedCipherSuiteNames.add(cipherSuite.getName());
            }
        }
        if (nullWithNullNull == null) {
            throw new RuntimeException("SSL_NULL_WITH_NULL_NULL cipher suite not found");
        }
        SSL_NULL_WITH_NULL_NULL = nullWithNullNull;

        SUPPORTED_CIPHER_SUITES =
                supportedCipherSuites.toArray(new CipherSuite[supportedCipherSuites.size()]);
        SUPPORTED_CIPHER_SUITE_NAMES =
                supportedCipherSuiteNames.toArray(new String[supportedCipherSuiteNames.size()]);
    ...
    private static CipherSuite getCipherSuiteByCode(int i1, int i2, int i3) {
        if (i1 != 0) {
            // Legacy SSLv2 cipher suite codes not supported
            return null;
        }
        int code = ((i2 & 0xff) << 8) | (i3 & 0xff);
        return SUITES_BY_CODE.get(code);
    }
