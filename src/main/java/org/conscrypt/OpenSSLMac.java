...
public abstract class OpenSSLMac extends MacSpi {
    ...
    private final void resetContext() {
        ...
        if (macKey != null) {
            NativeCrypto.EVP_DigestSignInit(ctxLocal.getContext(), evp_md, macKey.getPkeyContext());
        }
    ...
    protected void engineUpdate(byte[] input, int offset, int len) {
        ...
        NativeCrypto.EVP_DigestUpdate(ctxLocal.getContext(), input, offset, len);
    }

    @Override
    protected byte[] engineDoFinal() {
        final OpenSSLDigestContext ctxLocal = ctx;
        final byte[] output = NativeCrypto.EVP_DigestSignFinal(ctxLocal.getContext());
        resetContext();
        return output;
    }
