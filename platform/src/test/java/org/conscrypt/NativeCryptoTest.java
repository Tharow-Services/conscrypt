        ...
        Hooks sHooks = new ServerHooks(getServerPrivateKey(), getServerCertificates()) {
            ...
            public void afterHandshake(long session, long s, long c, Socket sock, FileDescriptor fd,
                ...
                try {
                    ...
                    // SSL renegotiation, which is what we expect.  If the exception isn't thrown,
                    // the SSL_write in the catch block won't execute, so the client.get() below
                    // won't complete and the test will fail.
