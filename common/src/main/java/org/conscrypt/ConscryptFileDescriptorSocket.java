...
class ConscryptFileDescriptorSocket extends OpenSSLSocketImpl
    ...
    public final void startHandshake() throws IOException {
        ...
        synchronized (ssl) {
            if (state == STATE_NEW) {
                LogUtils.log(
                        "Application (" + LogUtils.getName() + ", " + LogUtils.getUid()
                                + ") has started a SSL/TLS handshake with " +
                                "a remote connection endpoint (" + this.getHostname() + ":"
                                + this.getPort() + ")",
    ...
    public final void onSSLStateChange(int type, int val) {
        ...
        LogUtils.log("Application (" + LogUtils.getName() + ", " + LogUtils.getUid()
                + ") has finished a SSL/TLS handshake with " +
                ...
                try {
                    ...
                    if (peerCertChain != null) {
                        for (int i = 0; i < peerCertChain.length; i++) {
                            LogUtils.log(
                                    "Chain verification failed. Cert[" + i + "]: "
                                            + peerCertChain[i].getSubjectDN().getName() +
                                            " Issuer: " + peerCertChain[i].getIssuerDN().getName()
                                            + " Reason: " + e.getMessage(),
                                    false, LogUtils.AUDIT_LOG_WARNING,
    ...
    private void shutdownAndFreeSslNative() throws IOException {
        try {
            ...
            LogUtils.log(
                    "Application (" + LogUtils.getName() + ", " + LogUtils.getUid()
                            + ") has finished a SSL/TLS session with " +
                            "a remote connection endpoint (" + this.getHostname() + ":"
                            + this.getPort() + ")",
                    true, LogUtils.AUDIT_LOG_NOTICE, "ConscryptFileDescriptorSocket");
        } catch (IOException ignored) {
