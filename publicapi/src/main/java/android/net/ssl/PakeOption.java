...
public final class PakeOption {
    ...
    private PakeOption(String algorithm, Map<String, byte[]> messageComponents) {
        ...
        Map<String, byte[]> copy = new HashMap<>(messageComponents.size());
        for (Map.Entry<String, byte[]> entry : messageComponents.entrySet()) {
            copy.put(entry.getKey(), entry.getValue().clone());
        }
        this.messageComponents = Collections.unmodifiableMap(clone);
    }
